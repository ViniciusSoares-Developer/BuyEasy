<header class="container-fluid sticky-top bg-primary bg-gradient top-0 mb-4">
    <section class="row justify-content-center">
        ...
        <section class="col-6 col-sm-6 my-2">
            <div class="row justify-content-center m-0">
                <a href="?page=initial" class="col-12 col-sm-4 p-0 py-2 d-flex justify-content-center">
                    <img src="assets/images/logoHeader.png" alt="" class="w-100 logo"
                        style="min-width: 200px; filter: drop-shadow(0 0 .2rem white);">
                </a>
            </div>
        </section>
        ...
        <section class="col-sm-3 my-2">
            <div class="w-100 h-100 d-flex align-items-center justify-content-center">
                <a href="?page=user&idU=<?= $_SESSION['user']['id'] ?>" class="me-2"><img
                        src="<?= $_SESSION['user']['image_path'] ?>" class="rounded-circle img1:1 border border-primary"
                ...
                <div class="dropdown">
                    ...
                    <ul class="dropdown-menu">
                        ...
                        <li>
                            <a class="dropdown-item" href="?page=list_buy"><i class="fas fa-shopping-cart"></i> Compras
                                Realizadas</a>
                        </li>
                        <li>
...
<div class="offcanvas offcanvas-start text-primary" id="cart">
    ...
    <div class="offcanvas-body">
        ...
        <div class="card mb-3" style="max-width: 540px;">
            <div class="row g-0">
                ...
                <div class="col-7">
                    <div class="card-body">
                        <h5 class="card-title">
                            <?= substr($item['name'], 0, 17)."..." ?>
                        </h5>
                        <p class="card-text">R$: <?= $item['price'] ?>
                        </p>
                    </div>
                </div>
                <div class="col-12">
                    <form method="GET">
                        ...
                        <button type="submit" name="index" value="<?= $index ?>"
                            class="btn btn-outline-secondary w-100">Atualizar</button>
                    </form>
                </div>
            </div>
        </div>
        <?php endforeach; ?>
        <?php else: ?>
            <h5>Vazio <i class="fas fa-spinner"></i></h5>
            <p>Adicione itens para vê a lista aqui</p>
        <?php endif; ?>
    </div>
    <div class="offcanvas-footer">
        <p class="fs-4 p-0 m-0">Total: R$
            <?php
        $total = 0;
        ...
        <form method="POST">
            <select class="form-select" name="discount" id="">
                ...
                <option value="10" <?php if ($total < 100)
                echo "disabled" ?>>Desconto de 10% para valores acima de R$100
                </option>
                <option value="25" <?php if ($total < 200)
                echo "disabled" ?>>Desconto de 25% para valores acima de R$200
                </option>
                <option value="40" <?php if ($total < 500)
                echo "disabled" ?>>Desconto de 40% para valores acima de R$500
                </option>
            </select>
            <button type="submit" name="cart" value="confirm" class="btn btn-success w-100" <?php
                if (empty($_SESSION['cart']))
                echo "disabled" ?>>Finalizar compra</button>
        </form>
        <?php else: ?>
